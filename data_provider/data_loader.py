...
# class SWATSegLoader(Dataset):
    def __init__(self, root_path, flag='train', size=None, gl=0,
                  data_path='ETTh1.csv',scale=True, labels=None, step=1):
        self.flag = flag
        self.step = step
...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', size=None, gl=0,
                  data_path='ETTh1.csv',scale=True, labels=None, ad=False):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
    ...
    def __read_data__(self):
        ...
        cols_data = df_raw.columns[1:]
        if self.labels:
            cols_data = [col for col in cols_data if col in self.labels]
        df_data = df_raw[cols_data]
        ...
        if self.ad:
            if self.scale:
                train_data = df_data[border1s[0]:border2s[0]]
                self.scaler.fit(train_data.values[:, :-1])
                data = self.scaler.transform(df_data.values[:, :-1])
            else:
                data = df_data.values[:, :-1]
            self.data_x = data[border1:border2]
